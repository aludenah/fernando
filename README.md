...
- La nota se calcula a partir de las respuestas ya guardadas, también las anteriores a esta actualización. Se mantienen los identificadores, alternativas y bases de datos del capítulo 1. El servicio de avance incorpora automáticamente las preguntas publicadas en los archivos del curso después de instalar una vez la versión con catálogo automático; la calificación se calcula en la web.
...
4. Ejecuta `node scripts/build-progress-service.mjs` y las comprobaciones del repositorio. Publica el contenido actualizado en GitHub. Una vez instalada la versión con catálogo automático, el servicio detecta los capítulos publicados sin volver a pegar código ni implementar cada capítulo. [Instalación y comprobación del servicio](integrations/progress/README.md).

`lesson` conserva el primer capítulo para compatibilidad; `additionalLessons` contiene los siguientes. Cada tarea usa su `chapterId`; no reutilices identificadores de preguntas o temas. La web consulta los campos admitidos por el servicio antes de enviar contenido nuevo. Si el servicio sigue en la versión anterior, el capítulo 1 continúa sincronizándose y el capítulo 2 conserva sus respuestas pendientes en el dispositivo, con un aviso visible. Al actualizar la misma implementación, los pendientes que aún existan en el navegador original se enviarán sin volver a responder. Si se perdió ese navegador antes de sincronizar, no hay una copia remota de esas respuestas.
